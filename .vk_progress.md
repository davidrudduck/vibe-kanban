...
1. Start Session 3: Activity Feed Query & Frontend
2. Focus on `crates/db/src/models/activity_feed.rs` first
3. Update the SQL query: `COALESCE(t.activity_at, t.updated_at) AS activity_at`
4. Update ORDER BY clause to use new column
5. Run `npm run generate-types` after Rust changes
6. Update frontend component to use `activity_at`

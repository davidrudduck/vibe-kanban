...
## Overview
Implementing remote task attempt operations for the swarm/hive setup. All 7 steps complete.
...
### Step 7: Connection Status Badge ✓
**Frontend:**
- [x] Created `ConnectionStatusBadge` component in `frontend/src/components/common/ConnectionStatusBadge.tsx`
  - Shows status: Local | Direct | Relay | Disconnected
  - Styled following `NodeStatusBadge` pattern
  - Includes icons (Monitor, Wifi, Cloud, WifiOff) and tooltips
- [x] Created `useRemoteConnectionStatus` hook in `frontend/src/hooks/useRemoteConnectionStatus.ts`
  - Returns `status`, `connectionInfo`, `isLoading`, `error`
  - Fetches connection info for remote tasks
  - Returns 'local' for local tasks
- [x] Integrated badge into `AttemptHeaderActions` component
  - Shows badge only for remote tasks (`task?.is_remote && connectionStatus !== 'local'`)
  - Placed before the view toggle buttons
- [x] Added i18n translations for remote connection tooltips in diff panel

## Remaining Work
None - all 7 steps complete.
...
| `frontend/src/components/panels/AttemptHeaderActions.tsx` | Integrated `ConnectionStatusBadge` for remote tasks |
| `frontend/src/components/common/ConnectionStatusBadge.tsx` | New component showing connection status |
| `frontend/src/hooks/useRemoteConnectionStatus.ts` | New hook for determining remote connection status |
| `frontend/src/pages/ProjectTasks.tsx` | Pass `task` prop to `DiffsPanel` |
| `frontend/src/i18n/locales/en/tasks.json` | Added remote connection tooltip translations |
...
All 7 steps complete. Ready for review and merge.

Potential future enhancements:
- Add relay fallback when direct WebSocket connection fails
- End-to-end testing with actual multi-node swarm setup

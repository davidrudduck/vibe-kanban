...
1. Each node lists their `local projects` and all `swarm projects` on `/projects`
  - local projects are stored only in the local sqlite database
  - local projects are only accessible by that node
  - tasks of local projects are stored in the local sqlite database
  - swarm projects are stored in the hive database
  - tasks of swarm projects are stored in the hive database
  - when a local project becomes a swarm project, the tasks are synchronised to the hive
  - if a swarm project gets deleted from the hive:
    - the projects become local projects on the nodes linked to that swarm project
    - the tasks and task executions become owned by the node(s) where an execution has been run. 
    - if no execution has been run for a task, the user must decide which node to assign the task to
...
Launch sub agents in parallel to inspect code and look for race conditions, logic faults, old, legacy code that may be conflicting, needs for refactoring code, centralising commonly used functions to reduce technical debt and confusion.

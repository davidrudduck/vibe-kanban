...
## Acceptance Criteria
- [ ] `reject_if_remote(&deployment.db().pool, &task_attempt).await?;` added to handler
- [ ] Call placed immediately after `let task_attempt = ...` line
- [ ] Comment explaining why manual check is needed (bypass middleware)
- [ ] Handler continues to work for local task attempts
- [ ] Handler returns BadRequest for remote task attempts

...
```json profiles.json
{
  "executors": {
    ...
    "GEMINI": {
      "DEFAULT": { "GEMINI": { "model": "gemini-2.5-pro", "yolo": true } },
      "FLASH":   { "GEMINI": { "model": "gemini-2.5-flash", "yolo": true } }
    },
    "CODEX": {
...
<Tab title="GEMINI">
  ...
  Choose a Gemini model such as `"gemini-2.5-pro"` or `"gemini-2.5-flash"`
  </ParamField>
...
<Tab title="CODEX">
  ...
  Execution environment: `"auto"`, `"read-only"`, `"workspace-write"`, or `"danger-full-access"`
  </ParamField>

  <ParamField path="ask_for_approval" type="string">
  Approval level: `"unless-trusted"`, `"on-failure"`, `"on-request"`, or `"never"`
  </ParamField>

  <ParamField path="model_reasoning_effort" type="string">
  Reasoning depth: `"low"`, `"medium"`, `"high"`, or `"xhigh"`
  </ParamField>
  ...
  [View full documentation →](https://developers.openai.com/api/docs/models)
</Tab>

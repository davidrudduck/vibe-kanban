...
<Info>
Vibe Kanban supports two MCP transport modes:

- **stdio** for local MCP clients such as Claude Desktop, Raycast, or coding agents launched on the same machine
- **Streamable HTTP** at `/mcp` when `MCP_PORT` is configured for a running Vibe Kanban server

If you need HTTPS, terminate TLS in a reverse proxy or tunnel in front of the MCP HTTP endpoint.
</Info>
...
#### MCP server CLI flags

| Flag | Description |
|------|-------------|
| `--mode <global\|orchestrator>` | Launch mode. `global` (default) provides all tools. `orchestrator` scopes tools to the current workspace session. |
| `--host <HOST>` | Override the host used to connect to the Vibe Kanban backend. |
| `--backend-url <URL>` | Explicit backend URL (e.g. `http://127.0.0.1:3001`). When set, this takes precedence over all host/port discovery mechanisms. |
| `--http` | Use HTTP transport instead of stdio. Requires `--port`. |
| `--port <PORT>` | Port for the HTTP MCP server (only valid with `--http`). |

```json Example: explicit backend URL
{
  "mcpServers": {
    "vibe_kanban": {
      "command": "npx",
      "args": ["-y", "vibe-kanban@latest", "--mcp", "--backend-url", "http://127.0.0.1:3001"]
    }
  }
}
```
...
<Tip>
Use `--backend-url` when Vibe Kanban is running on a fixed port and you want to avoid the automatic port-file discovery, for example in CI environments.
</Tip>

### Option 3: HTTP / HTTPS Endpoint

When `MCP_PORT` is set, the Vibe Kanban backend also spawns an HTTP MCP server at:
...
```bash
HOST=0.0.0.0 BACKEND_PORT=3001 MCP_PORT=3002 pnpm run backend:dev:watch
```

This exposes the MCP endpoint on port `3002` while the backend API continues to run on port `3001`.

If you want to expose it over HTTPS, put a reverse proxy or tunnel in front of `/mcp` and terminate TLS there.
...
| `create_session` | Create a session in an existing workspace | None | `workspace_id`<br/>`executor`<br/>`name` | Session summary |
| `create_and_run_session` | Create a session and immediately run a prompt in one call | `executor`<br/>`prompt` | `workspace_id`<br/>`name` | `session_id`, `execution_id`, session summary, execution details |
...
| `get_session_history` | Return the full prompt and response history for a session | `session_id` | `include_soft_deleted` | Session summary and array of turns |

#### `create_and_run_session`

Combines `create_session` and `run_session_prompt` into a single call. Use this when you want to start a new coding task without the extra round-trip of creating a session first.

**Parameters**

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `executor` | string | Yes | Coding agent to use (e.g. `claude-code`, `gemini`). Same values as `start_workspace`. |
| `prompt` | string | Yes | Initial prompt to run. Must not be empty. |
| `workspace_id` | UUID | No | Workspace to create the session in. Inferred from context when running inside an orchestrator MCP. |
...
```json
{
  ...
  "session": { "id": "...", "workspace_id": "...", "executor": "claude-code", ... },
  "execution": { "id": "...", "status": "running", ... }
}
```

<Note>
If the session is created successfully but the prompt fails to start, the response includes the `session_id` so you can retry with `run_session_prompt` rather than accumulating orphaned sessions.
</Note>

#### `get_session_history`

Returns the ordered list of coding-agent turns for a session. Each turn corresponds to one prompt/response cycle.

**Parameters**

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `session_id` | UUID | Yes | Session to inspect. |
| `include_soft_deleted` | boolean | No | Include turns that have been dropped from the active history view. Defaults to `false`. |
  ...
  "turns": [
    {
      ...
      "final_message": "I've implemented the login endpoint in src/routes/auth.rs ...",
...
**Use case:** Review what an agent did in a past session before continuing work, or audit the sequence of prompts sent to an agent during an automated pipeline.

...
SERVER_PUBLIC_BASE_URL=http://your-server:9000
# Server port (must match the port in SERVER_PUBLIC_BASE_URL)
SERVER_PORT=9000
...
# PostgreSQL Connection Pool (optional, default: 20)
# Increase for production with many connected nodes
# VK_PG_MAX_CONNECTIONS=30
```
...
### Automatic Re-Linking on Connect

When a node connects (or reconnects) to the hive, it automatically re-registers all previously linked projects. This ensures:

- Projects linked before the node joined a hive are properly registered
- Projects remain visible on the hive's `/nodes/{node_id}` page after reconnection
- No manual re-linking is required after node restarts

The auto-linking process:

1. Node authenticates with hive via WebSocket
2. Hive sends list of known linked projects for this node
3. Node queries local database for all projects with `remote_project_id` set
4. For any project not in the hive's list, node sends `LinkProject` message
5. Hive registers the project-node relationship

<Note>
Auto-linking only registers local projects (where `is_remote = 0`). Projects that originated from the hive are not re-linked as they're already tracked by the hive.
</Note>

### Automatic Sync After Linking

Once a project is linked, the following data syncs automatically:

| Data | Direction | Method |
|------|-----------|--------|
| Tasks (hive-created) | Hive → Node | ElectricSQL + WebSocket |
| Tasks (locally-created) | Node → Hive | WebSocket (`TaskSync`) |
| Labels | Bidirectional | WebSocket (`LabelSync`) |
| Task Attempts | Node → Hive | HiveSyncService (background) |
| Execution Processes | Node → Hive | HiveSyncService (background) |
| Log Entries | Node → Hive | HiveSyncService (batched) |

The **HiveSyncService** runs automatically in the background (every 5 seconds) to sync execution data. This is offline-first: data is queued locally if the Hive is unreachable and synced when connection is restored.

### Local vs Remote Tasks

Tasks have an `is_remote` flag that determines their editability:

| Type | `is_remote` | Editable | Description |
|------|-------------|----------|-------------|
| Local | `0` | Yes | Created on this node |
| Remote | `1` | No (view only) | From hive or another node |

When you create a task locally on a node that's connected to a hive:
1. The task is created with `is_remote = 0` (editable)
2. When the task has execution attempts, it's synced to the hive
3. The hive assigns a `shared_task_id` so execution data can be tracked
4. The task remains editable on the originating node
...
### 3a. Dispatching to Specific Nodes

When creating a task attempt, you can optionally specify a target node:
...
curl -X POST http://localhost:3000/api/task-attempts \
  ...
  -d '{
    ...
    "executor": "CLAUDE_CODE",
    "base_branch": "main",
    "target_node_id": "node-uuid-to-execute-on"
  }'
```

When a task is dispatched to a node, the Hive tracks which node is executing via `executing_node_id`. This enables:
- Viewing which node is running each task
- Routing log streaming requests to the correct node
...
5. Verify `SERVER_PORT` in `.env.remote` matches the port in `VK_HIVE_URL`
...
1. Check node is connected to hive (WebSocket connection active)
2. Verify label has been modified after linking project
3. Check node logs for `LabelSync` messages
4. Labels sync every 5 seconds - wait for next sync cycle

### Local Tasks Showing as Read-Only

If tasks you created locally appear as read-only (greyed out, no edit options):

1. Check `is_remote` flag in database: should be `0` for local tasks
2. Verify project `is_remote` flag: should be `0` for locally-owned projects
3. If incorrect, a migration may need to run to fix the flag
4. Check logs for "sync from shared task" events that may have overwritten the flag

### User Keeps Getting Logged Out

If you're repeatedly asked to sign in again:

1. Check logs for `failed to refresh OAuth credentials` messages
2. Look for `received auth error from server` entries
3. Verify the hive server is using a consistent `VIBEKANBAN_REMOTE_JWT_SECRET`

Common causes:
- Refresh token expired (user inactive for extended period)
- Hive server restarted with different JWT secret
- Clock skew between node and hive (check system time)

See [Auth Token Management](/architecture/auth-token-management) for technical details on token refresh logic.
...
# Network
SERVER_PUBLIC_BASE_URL=http://localhost:9000
SERVER_PORT=9000
...
# Connection Pool (optional - increase for many nodes)
# VK_PG_MAX_CONNECTIONS=30
EOF
